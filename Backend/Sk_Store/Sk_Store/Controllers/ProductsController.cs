...
namespace Sk_Store.Controllers // Đảm bảo namespace này đúng với project của bạn
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        /// <summary>
        /// Tạo một sản phẩm mới với upload ảnh (Yêu cầu quyền Admin).
        /// </summary>
        /// <param name="createProductDto">DTO chứa thông tin và file ảnh để tạo sản phẩm.</param>
        /// <returns>Thông tin sản phẩm vừa tạo hoặc lỗi nếu có.</returns>
        [HttpPost("upload")]
        [Authorize(Roles = "Admin")]
        [ProducesResponseType(typeof(ProductDetailDto), StatusCodes.Status201Created)]
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status403Forbidden)]
        public async Task<IActionResult> CreateProductWithFiles([FromForm] CreateProductWithFilesDto createProductDto)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var createdProduct = await _productService.CreateProductWithFilesAsync(createProductDto);
                if (createdProduct == null)
                {
                    return BadRequest(new { message = "Không thể tạo sản phẩm. Vui lòng kiểm tra lại thông tin đầu vào." });
                }
        ...
        // Thêm method này vào ProductsController class
        ...
        [HttpPut("{id}/upload")]
        [Authorize(Roles = "Admin")]
        public async Task<ActionResult<ProductDetailDto>> UpdateProductWithFiles(
            int id,
            [FromForm] UpdateProductWithFilesDto updateDto)
        {
            try
            {
                var result = await _productService.UpdateProductWithFilesAsync(id, updateDto);
                return Ok(result);
            }
            ...
            {
                return StatusCode(500, new { message = "Đã xảy ra lỗi khi cập nhật sản phẩm: " + ex.Message });
            }
        }
        ...
        public async Task<ActionResult> DeleteProductImage(int productId, int imageId)
        {
            ...
            {
                return StatusCode(500, new { message = "Đã xảy ra lỗi khi xóa ảnh: " + ex.Message });
            }
        }
    }
}
